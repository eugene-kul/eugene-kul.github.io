...
export const patchNode = (node, vNode, nextVNode) => {
    ...
    if (
        ...
        // Заменяем ноду на новую, если как минимум одно из значений равно
        // строке или числу и эти значения не равны друг другу
        if (vNode !== nextVNode) {
        ...
        // Если два значения - это строки или число и они равны,
...
const patchProp = (node, key, value, nextValue) => {
    ...
    // Стоит отметить, что setAttribute преобразует значение в строчку, поэтому
    // если нужно хранить объекты, массивы или методы, то их необходимо сеттить
    // в node, например: const key = 'customArray'; const value = [1, 5];
    // node[key] = value;
...
export const patch = (nextVNode, node) => {
    if (!node) {
        if (nextVNode.props.hasOwnProperty('class')) {
            console.warn(
                `node for ${nextVNode.tagName}.${nextVNode.props.class} is ${typeof node}!`)
        }
        else {
            console.warn(
                `node for ${JSON.stringify(nextVNode)} is ${typeof node}`)
        }
    }
