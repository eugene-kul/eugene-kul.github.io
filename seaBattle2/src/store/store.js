    ...
    proxy () {
        ...
        return {
            set: function (target, property, value, receiver) {
                ...
                if (JSON.stringify(target[property]) ===
                ...
                if (self.status !== 'mutation') {
                    console.warn(
                        `Denied manual assignment of a value for ${property} in ${JSON.stringify(
                            target)}\n ` +
                ...
                self.events.publish(camelCase('change ' + property),
                    self.state)
            ...
            // 	if (property === "isProxy" || property === "toJSON") return
            // true  const prop = target[property]  if(typeof prop ==
            // 'undefined') return   if (typeof prop === "object" &&
            // !prop["isProxy"] && !(prop instanceof HTMLElement) ) {
            // target[property] = new Proxy(prop, self.proxy()); }  return
            // target[property] }
        }
    }
