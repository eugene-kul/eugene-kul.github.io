...
export const gameItem = (props) => {
    ...
    const shipDragStart = (e) => {
        ...
        const onMouseMove = e => {
            ...
            if (shipX <= 0) shipX = 0
            if (shipX > store.state.settings.fields.x - 1) shipX = store.state.settings.fields.x - 1
            if ((shipX + ship.s) >= store.state.settings.fields.x && !ship.r) shipX = store.state.settings.fields.x - ship.s

            if (shipY <= 0) shipY = 0
            if (shipY > store.state.settings.fields.y - 1) shipY = store.state.settings.fields.y - 1
            if ((shipY + ship.s) >= store.state.settings.fields.y && ship.r) shipY = store.state.settings.fields.y - ship.s
