//@prepros-append js/jQuery.min.js
//@prepros-append js/retina.js
//@prepros-append js/galleryFB.js
//@prepros-append js/slick.js
//@prepros-append js/sliders.js
//@prepros-append js/script.js
//@prepros-append js/removeBlock.js
 ...
 */
 ...
 (function() {
	...
	var config = {
		// Возможность выбрать суффикс для 2х изображений
		retinaImageSuffix : '@2x',
		// Убедитесь, что Content-Type является изображением, прежде чем пытаться загрузить изображение @2x
		check_mime_type: true,
		// Изменение размера изображений с высоким разрешением до размеров исходного изображения в пикселях
...
(function (window, document, $, undefined) {
  ...
  var defaults = {
    ...
    // Установите значение false, если вам не нужно складывать несколько экземпляровнесколько
    closeExisting: false,
    ...
    // Должен отображать кнопку закрытия (используйте `btnTpl.smallBtn` шаблон) может быть true, false, "auto"
    smallBtn: "auto",

    // Должна отображаться панель инструментов (кнопки вверху) может быть true, false, "auto"
    toolbar: "auto",
    ...
    btnTpl: {
      ...
      arrowLeft: '<button data-gbox-prev class="gbox-button gbox-button--arrow_left" title="{{PREV}}">' +
        '<div><svg version="1.1" x="0px" y="0px" viewBox="0 0 477.175 477.175" style="enable-background:new 0 0 477.175 477.175;" xml:space="preserve"><path d="M145.188,238.575l215.5-215.5c5.3-5.3,5.3-13.8,0-19.1s-13.8-5.3-19.1,0l-225.1,225.1c-5.3,5.3-5.3,13.8,0,19.1l225.1,225c2.6,2.6,6.1,4,9.5,4s6.9-1.3,9.5-4c5.3-5.3,5.3-13.8,0-19.1L145.188,238.575z"/></svg></div>' +
        "</button>",

      arrowRight: '<button data-gbox-next class="gbox-button gbox-button--arrow_right" title="{{NEXT}}">' +
        '<div><svg version="1.1" x="0px" y="0px" viewBox="0 0 477.175 477.175" style="enable-background:new 0 0 477.175 477.175;" xml:space="preserve"><path d="M360.731,229.075l-225.1-225.1c-5.3-5.3-13.8-5.3-19.1,0s-5.3,13.8,0,19.1l215.5,215.5l-215.5,215.5c-5.3,5.3-5.3,13.8,0,19.1c2.6,2.6,6.1,4,9.5,4c3.4,0,6.9-1.3,9.5-4l225.1-225.1C365.931,242.875,365.931,234.275,360.731,229.075z"/></svg></div>' +
        "</button>",
...
if($('.slider-project-body').length>0){
	$('.slider-project-body').slick({
		infinite: true,
		arrows: true,
		dots: true,
		swipe: true,
		accessibility: false,
		nextArrow:'<button type="button" class="slick-next"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 31.49 31.49" style="enable-background:new 0 0 31.49 31.49;" xml:space="preserve"><path d="M21.205,5.007c-0.429-0.444-1.143-0.444-1.587,0c-0.429,0.429-0.429,1.143,0,1.571l8.047,8.047H1.111C0.492,14.626,0,15.118,0,15.737c0,0.619,0.492,1.127,1.111,1.127h26.554l-8.047,8.032c-0.429,0.444-0.429,1.159,0,1.587c0.444,0.444,1.159,0.444,1.587,0l9.952-9.952c0.444-0.429,0.444-1.143,0-1.571L21.205,5.007z"/></svg></button>',
		prevArrow:'<button type="button" class="slick-prev"><svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 31.494 31.494" style="enable-background:new 0 0 31.494 31.494;" xml:space="preserve"><path d="M10.273,5.009c0.444-0.444,1.143-0.444,1.587,0c0.429,0.429,0.429,1.143,0,1.571l-8.047,8.047h26.554c0.619,0,1.127,0.492,1.127,1.111c0,0.619-0.508,1.127-1.127,1.127H3.813l8.047,8.032c0.429,0.444,0.429,1.159,0,1.587c-0.444,0.444-1.143,0.444-1.587,0l-9.952-9.952c-0.429-0.429-0.429-1.143,0-1.571L10.273,5.009z"/></svg></button>',
		responsive: [
			{
				breakpoint: 767.98,
				settings: {
...
document.addEventListener("DOMContentLoaded", function(event) {
	...
	add = 'add',
	hasCls = 'hasCls',
	text = ['Читать полностью', 'Скрыть текст'];

	function refreshText(element,text) {
		document.querySelector(element).textContent = text;
	}

	//работа с классами
	function ClsList(element,cls,event) {
		let elementList = document.querySelectorAll(element);
		for (let item of elementList) {
			if (event === 'toggle') {item.classList.toggle(cls);}
			else if (event === 'add') {item.classList.add(cls);}
			else if (event === 'remove') {item.classList.remove(cls);}
			else if (event === 'hasCls') {
				item.classList.contains(cls);
				if (item.classList.contains(cls)) {return true}
				return false;
			}
		}
	}

	let pageNumbers = document.querySelectorAll('.pagination_item-numbers');
	let prevPage, nextPage;
	if (pageNumbers != null) {
		let i = 1;
		for (let itemNumber of pageNumbers) {
			itemNumber.setAttribute('href', 'indexPageProject'+i+'.html')
			if (itemNumber.classList.contains('active')) {
				if ((i-1) != 0) {
					ClsList('.pagination_prev','active',add);
					document.querySelector('.pagination_prev').setAttribute('href', 'indexPageProject'+(i-1)+'.html')
				}
				if ((i+1) <= pageNumbers.length) {
					ClsList('.pagination_next','active',add);
					document.querySelector('.pagination_next').setAttribute('href', 'indexPageProject'+(i+1)+'.html')
				}
				document.querySelector('.pageTarget').textContent = i;
				itemNumber.removeAttribute('href');
			};
			i++;
		}
	}
	...
	document.querySelector('.menu-icon').addEventListener('click', function(event) {
		...
		ClsList('.bottom-row_menu-list','active',toggle);
		ClsList('.bottom-row_overflow','active',toggle);
		ClsList('body','lock',toggle);
	});

	document.querySelector('.bottom-row_overflow').addEventListener('click', function(event) {
		ClsList('.bottom-row_overflow','active',remove);
		ClsList('.bottom-row_menu-list','active',remove);
		ClsList('.menu-icon','active',remove);
		ClsList('body','lock',remove);
	});

	let btnText = document.querySelectorAll('.show-text');
	if (btnText != null) {
		for (let btn of btnText) {
			...
			btn.addEventListener('click', function(event) {
				...
				if (prev.classList.contains('show')) {
					...
					scrollTo('.'+prev.classList[0],-150);
				}
				else if (prev.classList.contains('hide')) {
	...
	function scroll(){
		...
		if (scrollTop > 200) {
			ClsList('.buttonUp','active',add);
		} else {ClsList('.buttonUp','active',remove);}
	}

	document.querySelector('.buttonUp').addEventListener('click', function(event) {
		scrollTo('body');
	});

	//перенос русского текста
	(function () {
		let textList = document.querySelectorAll('.txt'),
	...
	//скролл до элемента через querySelector по уникальному class/id/teg. x - смещение скролла
	...
	//спойлер(акордион) (.spMain, spSub (height = 0), .oneBlock)
	...
	for (let item of spMain) {
		item.addEventListener('click', function(event) {
			...
			function hideAll() {
				...
				for (let itemSub of spSub) {
					itemSub.style.height = '0';
				}
			}
		});
	}
